document.addEventListener('DOMContentLoaded', function () {
    ...
    function init() {
        ...
        scene.background = null; // Sfondo trasparente per integrarsi con il design moderno
    ...
    function animate() {
        ...
        if (houseData.isGrowing) {
            ...
            // Optimize: Update scale instead of recreating geometry
            // Base scaling
            // Original geometry height is houseData.initialBaseHeight (0.1)
            const baseScale = houseData.currentBaseHeight / houseData.initialBaseHeight;
            houseData.baseMesh.scale.y = baseScale;
            houseData.baseMesh.position.y = (houseData.initialBaseHeight * baseScale) / 2;

            // Roof scaling
            // Original geometry height is houseData.initialRoofHeight (0.1)
            const roofScale = houseData.currentRoofHeight / houseData.initialRoofHeight;
            houseData.roofMesh.scale.y = roofScale;
            // Roof position depends on base height
            houseData.roofMesh.position.y = houseData.currentBaseHeight + (houseData.initialRoofHeight * roofScale) / 2;
        }
